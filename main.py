...
# Right after a time, these sites show either a single digit (foreUP-style:
# how many golfers can currently join) or a range like "1-4" / "1 - 4"
# (GolfNow/CPS-golf-style: the group sizes still bookable). Either way, the
# largest single digit 1-4 found in a short window right after the time is
# the max group size still open for that slot — a bare or range-topping "4"
# means nobody's booked any of the four spots yet. Digits immediately
# preceded by '$' or another digit are excluded so price text (e.g. a "2" in
# "$2.99 fee") never gets mistaken for a slot count.
SLOT_COUNT_RE = re.compile(r'(?<![\d$])([1-4])(?!\d)')
SLOT_COUNT_WINDOW = 60  # characters to look ahead from the end of a time match
...
def max_available_slots(text, after_index, window=SLOT_COUNT_WINDOW):
    """Largest single digit 1-4 found shortly after a time match — the max
    group size still bookable for that slot. Returns None if no such digit
    is found nearby (format didn't match what we expect for this course)."""
    snippet = text[after_index:after_index + window]
    digits = [int(d) for d in SLOT_COUNT_RE.findall(snippet)]
    return max(digits) if digits else None


def build_url(course, target_date):
    """For courses whose booking page takes the date as part of the URL."""
    nav = course.get("nav_style")
    if nav in ("url_param", "hash_param"):
        ...
        base = course["url"]
        if nav == "hash_param":
            # GolfNow's search page is a client-side app that reads its date
            # from the URL's hash fragment (after '#'), not a real query
            # string — a '?date=...' is silently ignored by its JS router.
            sep = "&" if "#" in base else "#"
        else:
            sep = "&" if "?" in base else "?"
...
def fetch_page_text(playwright, course, target_date, debug=False):
    ...
    try:
        ...
        page.wait_for_timeout(4000)  # let the page's JS render the tee sheet
...
def check_course(playwright, course, watches_for_course, state, ntfy_topic, debug=False):
    ...
    for target_date in sorted(date_windows):
        ...
        matching = set()
        skipped_low_slots = 0
        required_slots = course.get("min_available_slots", 4)
        for m in TIME_RE.finditer(text):
            parsed = parse_time_str(m.group(0))
            if not parsed or not in_time_window(parsed[0], parsed[1], start_hour, end_hour):
                continue
            slots = max_available_slots(text, m.end())
            if slots is None or slots < required_slots:
                skipped_low_slots += 1
                continue
        ...
        if new_times:
            ...
            print(f"  [ok] {name} {date_key}: {len(matching)} matching time(s) with "
                  f"{required_slots}+ open, nothing new "
                  f"({skipped_low_slots} time(s) skipped for too few open slots)")
